...
4. **Architectural Refactoring** — compile-time configs centralised under strict `Must` patterns; artifact structs moved next to their owning agents (Locality of Behaviour); `SelectMigrationStrategy` replaced by `Registry.TryInOrder`; agent units decoupled; Eino graph widened to 8 functional `AddLambdaNode` calls (archaeologist, analyzer, planning, translator, reviewer, validator, verdict_panel, recruiter) + 2 checkpoint barriers (`save_translator_ckpt`, `save_validator_ckpt`) = 10 nodes total; default AST provider is `abcoder-mcp`; per-project compilation is binary Pass/Fail; ASD-STE100 phrasing applied across user-facing copy; TUI is idiomatic Charm (Bubble Tea + Bubbles + Lip Gloss + Glamour). The "8 nodes" wording in the original close-out is shorthand for the 8 functional nodes — see `[[1.0.0 ADR-2026-09-26-Vault-Lint-Extension]]` and `[[1.0.0 Architecture §4]]` for the canonical 10-node topology statement.
