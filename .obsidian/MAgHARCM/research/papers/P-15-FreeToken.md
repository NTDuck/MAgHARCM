...
**Cited by**: [[primitives/Primitives-Index]] entry as the empirical justification for MAgHARCM's "local-first" execution stance (used to support the rationale behind the chunked translator ([[PRIM-23]]) and conversable checkpoints ([[PRIM-28]]) running against local open-weight models rather than hosted APIs).
