...
- **Vault coverage gains** (catalog-corrected: PRIM titles must match `[[1.0.0 PRIM-NN]]` catalog; P-153 / P-154 / P-155 / P-156 re-routed to existing-matching primitives):
  - `[[1.0.0 PRIM-22]]` Four Phases of Comprehension: P-153 (CoReX context-aware refinement-based slicing = Structure-phase mechanism complementing P-133 ADI Frame Lifetime Trace + P-129 LλMDA partial-PDG) + P-155 (CodeCureAgent agentic warning-classification = Observation-phase substrate) + P-156 (TestWeaver backward slicing + execution annotation). Three new comprehension-mechanism anchors (one strict-mechanism; two LLM-augmented static-analysis).
  - `[[1.0.0 PRIM-31]]` Iterative Retrieval Refinement: P-153 (CoReX refinement-based slicing = iterative retrieval refinement substrate) + P-154 (TransAgent execution-aligned critic feedback = retrieval refinement by execution-behaviour comparison) + P-156 (TestWeaver close-test retrieval = iterative retrieval refinement for test-generation agents). Three new retrieval-refinement anchors.
  - `[[1.0.0 PRIM-23]]` Chunked Translation: P-154 (TransAgent multi-agent LLM translation + execution-aligned critic + iterative repair loop; extends the P-124/P-145/P-151 single-LLM translation lineage to multi-agent collaboration). One new multi-agent-translation anchor.
  - `[[1.0.0 PRIM-29]]` Dynamic Iteration Recruiter: P-154 (iterative repair loop as iteration-recruiter anchor with execution-aligned critic feedback) + P-155 (three-step build+test validation = iteration-recruiter with concrete acceptance heuristic) + P-156 (close-test retrieval + execution annotation = retrieval-and-recruitment substrate). Three new iteration-recruiter anchors.
  - `[[1.0.0 PRIM-13]]` Adversarial Test-Weakening Guard: P-155 (CodeCureAgent agentic warning-classification + repair twin-objective extends the static-analysis validation lineage). One new agentic-static-analysis anchor.
