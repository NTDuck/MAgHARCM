...
| `[[1.0.0 PRIM-13]]` | Adversarial Test Guard | AdvTestGen [[1.0.0 P-25]]; [[1.0.0 P-56]] Zan ([[1.0.0 P-56]]) Multi-SWE-bench | Mutation Testing & Assertion Invariants, [[1.0.0 P-48]] Jia & [[1.0.0 P-48]] Mutation Survey; multilingual adversarial benchmark; [[1.0.0 P-155]] CodeCureAgent agentic warning-classification + repair (wave-23) | `internal/agents/validator.go` |
...
| `[[1.0.0 PRIM-21]]` | Migration Strategy Selection | Müller et al. [[1.0.0 P-06]]; [[1.0.0 P-57]] Speculative Decoding; [[1.0.0 P-84]] s1 test-time scaling; [[1.0.0 P-91]] Snell test-time compute allocation; [[1.0.0 P-98]] Large Language Monkeys; [[1.0.0 P-108]] EAGLE-3 training-time-test draft model (SLM speculative decoding path); [[1.0.0 P-114]] Medusa (Cai 2024); [[1.0.0 P-122]] ReasoningBank strategy-distilled persistent memory (wave-16); [[1.0.0 P-123]] CodeChemist confidence-gated cross-lingual I/O oracle switching (wave-17); [[1.0.0 P-126]] ARC-Decode risk-bounded acceptance (wave-18); [[1.0.0 P-135]] SPECS continuous draft<->target budget knob (wave-19); [[1.0.0 P-137]] SuffixDecoding model-free suffix-tree draft path (wave-19); [[1.0.0 P-140]] Panta coverage-driven test prioritisation knob (wave-19); [[1.0.0 P-141]] KVFlow agent-activation-time scheduling signal (wave-19); [[1.0.0 P-145]] TerraMod knowledge-augmented migration context (wave-20); [[1.0.0 P-147]] SpecKV draft-model-driven KV lookahead for adaptive gamma controller (wave-21); [[1.0.0 P-148]] LookaheadKV parameter-efficient LoRA-modules on target model for per-strategy parameter-efficient tuning (wave-21); [[1.0.0 P-149]] SSD/Saguaro asynchronous draft-verify pipeline (wave-21) | Legacy Migration Frameworks (Müller); [[1.0.0 P-52]] Self-Consistency (strategy-level ensemble); speculative decoding strategy registry; wait-token budget per strategy attempt; compute-optimal allocation across strategies; sampling + verifier as cheapest strategy; EAGLE-3 training-time-test draft = 4B-target SLM speculative path; Medusa multi-head drafting as alternative strategy to EAGLE-3; ARC-Decode risk-budget knob for SLM-bandit acceptance; SPECS continuous draft<->target budget as compute-optimal allocation; SuffixDecoding model-free draft = no-cost drafter slot; Panta coverage-driven prioritisation knob for try-and-fail ordering; KVFlow agent-activation-time signal as scheduling knob; TerraMod knowledge-augmented context for legacy-specific strategy selection; SpecKV adaptive gamma controller = gamma between draft-KV-eviction-cost and compressed-context-quality trade-off; LookaheadKV parameter-efficient per-strategy tuning as ensemble-head; SSD/Saguaro async pipeline as compute-mask-overlap strategy; [[1.0.0 P-157]] ReST-KV output-reconstruction eviction strategy (wave-24) | `internal/agents/strategy.go` |
| `[[1.0.0 PRIM-22]]` | Four Phases Comprehension | Foltz ([[1.0.0 P-40]]) [[1.0.0 P-30]]; canonical hop-1 [[1.0.0 P-40]]; [[1.0.0 P-51]] Cassano ([[1.0.0 P-51]]) Can It Edit; [[1.0.0 P-54]] Abdin ([[1.0.0 P-54]]) Phi-3; [[1.0.0 P-55]] Schick ([[1.0.0 P-55]]) SLM Few-Shot; [[1.0.0 P-56]] Zan ([[1.0.0 P-56]]) Multi-SWE-bench; [[1.0.0 P-80]] StreamingLLM (long-context SLM mitigation); [[1.0.0 P-90]] Wei CoT; [[1.0.0 P-94]] LIMA curation hypothesis; [[1.0.0 P-96]] Zero-Shot CoT; [[1.0.0 P-99]] BIG-Bench Hard; [[1.0.0 P-100]] Decomposed Prompting (SLM multi-agent); [[1.0.0 P-110]] GraphCoder / CodeGraphRAG; [[1.0.0 P-113]] AutoCodeRover (Zhang 2024); [[1.0.0 P-114]] Medusa (Cai 2024); [[1.0.0 P-115]] OpenHands/CodeAct (Wang 2024); [[1.0.0 P-116]] Aider (Gauthier 2024-2025); [[1.0.0 P-117]] RepoCoder (Zhang ICLR 2023); [[1.0.0 P-118]] SWE-bench Lite (Jimenez 2024); [[1.0.0 P-119]] SWE-Rebench (Badertdinov NeurIPS 2025); [[1.0.0 P-120]] SWE-smith (Yang NeurIPS 2025 spotlight); [[1.0.0 P-121]] BFCL (Patil ICML 2025); [[1.0.0 P-128]] KVzip query-agnostic context reconstruction (wave-18); [[1.0.0 P-129]] LλMDA (wave-18); [[1.0.0 P-130]] SSAR (wave-18); [[1.0.0 P-133]] ADI Frame Lifetime Trace (wave-18); [[1.0.0 P-138]] RepairKV post-compression KV repair for long-context agentic retrieval (wave-19); [[1.0.0 P-139]] TypePro inter-procedural type inference for Recognition + Explanation (wave-19); [[1.0.0 P-140]] Panta static+dynamic signal for untested-path search (wave-19); [[1.0.0 P-142]] NESA Datalog policy for Reorganization (wave-20); [[1.0.0 P-143]] HalluShield Insight-phase summary fidelity (wave-20); [[1.0.0 P-144]] TraceCoder Observation phase via trace instrumentation (wave-20); [[1.0.0 P-145]] TerraMod Reorganization via external knowledge context (wave-20); [[1.0.0 P-150]] TestPrune coverage-driven context pruning as Observation substrate (wave-21) | DR. JONES cognitive traversal model (Foltz 2023 [[1.0.0 P-40]]) — Decomposition + Recognition + Organisation + Navigation + Explanation + Search; SLM-aware comprehension substrate via StreamingLLM attention sinks + Phi-3 chat-format alignment + CoT trace decomposition; CodeAct action format simplifies ACI for SLM tool-call generation; Aider repo-map as first-class prompt component; RepoCoder retrieve-then-regenerate loop; SWE-bench Lite per-instance comprehension-iteration budget; SWE-Rebench per-instance provenance gate as Search-phase contamination filter; SWE-smith procedural AST mutations as bug-topology-aware Anchoring phase; BFCL AST-based per-turn contract check for tool-call comprehension; KVzip query-agnostic compression as Observation substrate (replaces StreamingLLM windowed attention for long-context retrieval); LλMDA LLM-aided partial PDG = Recognition + Explanation LLM-augmented extension; SSAR structural+semantic hybrid = Reorganization substrate; ADI Frame Lifetime Trace = function-level Observation-phase dynamic substrate; RepairKV post-compression repair = long-context Observation repair; TypePro inter-procedural slicing = Recognition + Explanation static enrichment; Panta static+dynamic hybrid = untested-path Observation signal; NESA Datalog policy = Reorganization substrate decomposition; HalluShield SLM-grounded speculative-decoding defence = Insight-phase summary fidelity; TraceCoder Observation phase via trace instrumentation; TerraMod Reorganization via external knowledge context; TestPrune coverage-driven context pruning = Observation-phase regression-test minimisation substrate; [[1.0.0 P-152]] Hallu-Eval hallucination evaluation substrate (wave-22); [[1.0.0 P-153]] CoReX context-aware refinement-based slicing closes the strict comprehension-mechanism slot (wave-23); [[1.0.0 P-155]] CodeCureAgent observation-phase substrate (wave-23); [[1.0.0 P-156]] TestWeaver backward slicing + execution annotation (wave-23) | `internal/agents/comprehension.go` |
| `[[1.0.0 PRIM-23]]` | Chunked Translation | ChatDev [[1.0.0 P-12]], MetaGPT [[1.0.0 P-11]]; [[1.0.0 P-53]] Lost in the Middle; [[1.0.0 P-101]] Least-to-Most Prompting; [[1.0.0 P-111]] SWE-bench original (Jimenez ICLR 2024); [[1.0.0 P-113]] AutoCodeRover (Zhang 2024); [[1.0.0 P-120]] SWE-smith (Yang NeurIPS 2025 spotlight) | Bounded-Context Translation; LtM chained-decomposition for cross-chunk state propagation; original SWE-bench multi-file patch chunking as evaluation signal; AutoCodeRover patch chunking as synthesis unit; SWE-smith AST-rewrite strategies as adversarial test-synthesis training-corpus analogue; [[1.0.0 P-151]] SmartC2Rust three-signal feedback loop (wave-22); [[1.0.0 P-154]] TransAgent multi-agent pipeline + execution-aligned critic (wave-23) | `internal/agents/chunked_translator.go` |
...
| `[[1.0.0 PRIM-31]]` | Iterative Retrieval Refine | RepoCoder [[1.0.0 P-13]]; [[1.0.0 P-53]] Lost in the Middle; [[1.0.0 P-56]] Zan ([[1.0.0 P-56]]) Multi-SWE-bench; [[1.0.0 P-57]] Leviathan ([[1.0.0 P-57]]) Speculative Decoding; [[1.0.0 P-78]] EAGLE-3 multi-layer fusion; [[1.0.0 P-80]] StreamingLLM attention sinks; [[1.0.0 P-108]] EAGLE-3 training-time-test draft model (SLM speculative decoding path); [[1.0.0 P-110]] GraphCoder / CodeGraphRAG; [[1.0.0 P-114]] Medusa (Cai 2024); [[1.0.0 P-115]] OpenHands/CodeAct (Wang 2024); [[1.0.0 P-116]] Aider (Gauthier 2024-2025); [[1.0.0 P-117]] RepoCoder (Zhang ICLR 2023); [[1.0.0 P-119]] SWE-Rebench (Badertdinov NeurIPS 2025); [[1.0.0 P-120]] SWE-smith (Yang NeurIPS 2025 spotlight); [[1.0.0 P-121]] BFCL (Patil ICML 2025); [[1.0.0 P-128]] KVzip (wave-18); [[1.0.0 P-132]] SemRef (wave-18); [[1.0.0 P-133]] ADI (wave-18); [[1.0.0 P-134]] RelayCaching cross-agent prefill reuse (wave-19); [[1.0.0 P-138]] RepairKV second-chance correction mechanism for evicted KV rows (wave-19); [[1.0.0 P-141]] KVFlow workflow-aware eviction for multi-agent retention (wave-19); [[1.0.0 P-147]] SpecKV draft-model-driven lookahead for eviction importance (wave-21); [[1.0.0 P-148]] LookaheadKV parameter-efficient LoRA-modules on target model (wave-21); [[1.0.0 P-149]] SSD/Saguaro asynchronous draft-verify pipeline (wave-21); [[1.0.0 P-150]] TestPrune coverage-driven regression-test minimisation (wave-21) | EAGLE-3 multi-layer fusion as SLM-aware prefill acceleration; RepoCoder retrieve-then-regenerate loop; SWE-Rebench provenance-driven retrieval exclusion complements feedback-driven refinement; SWE-smith PR-mirrored ground-truth fixes as realistic-retrieval training substrate; BFCL call-pattern templates (serial/parallel/multi-turn) for retrieval-refinement training; KVzip query-agnostic compression for iterative retrieval loop (replaces StreamingLLM attention sinks for long-context); SemRef iterative semantic refinement as feedback-driven loop; ADI Frame Lifetime Trace as observation-driven retrieval trigger; RelayCaching cross-agent prefill reuse as multi-agent retrieval accelerator; RepairKV post-compression repair for long-context retrieval; KVFlow workflow-aware eviction for multi-agent retention; SpecKV draft-model lookahead for eviction importance; LookaheadKV parameter-efficient LoRA-modules for per-agent LoRA; SSD/Saguaro async draft-verify for parallel retrieval; TestPrune coverage-driven regression-test minimisation as input-compression substrate; [[1.0.0 P-151]] SmartC2Rust three-signal feedback substrate (wave-22); [[1.0.0 P-153]] CoReX refinement-as-iterative-retrieval (wave-23); [[1.0.0 P-154]] TransAgent execution-aligned critic feedback (wave-23); [[1.0.0 P-156]] TestWeaver close-test retrieval (wave-23); [[1.0.0 P-157]] ReST-KV output-reconstruction-governed eviction (wave-24) | `internal/agents/iter_retrieval.go` |
...
- Sprint 2026-09-07 (iter-2) audit block: `.obsidian/MAgHARCM/primitives/Primitives-Index.md` §Sprint 2026-09-07 (Wave-19)
## 12. Wave-21 SLM-Era Anchors (2026-09-07 iter-4)

4 ACCEPT + 3 REJECT + 1 UNVERIFIED = 8 triaged. Wave-21 reinforces the KV-cache × speculative-decoding × SLM-scale loop: P-147 SpecKV draft-model-driven lookahead for KV cache eviction, P-148 LookaheadKV parameter-efficient LoRA-modules on target model, P-149 SSD/Saguaro asynchronous draft-verify pipeline. P-150 TestPrune coverage-driven regression-test minimization closes a different SLM-comprehension substrate slot (coverage-driven context pruning). Watchlist: W1 SWE-TRACE carried Wave-19 W2 → Wave-20 W1 → Wave-21 W1 (NeurIPS 2026 notifications pending 2026-09-24).

### 12.1 Accepted anchors

- `[[1.0.0 P-147]]` **SpecKV: Draft-Model-Driven KV Cache Eviction** (Galim et al., ICLR 2026, OpenReview 0vbYakkECY, arXiv:2506.08373) — small draft model performs lookahead to predict KV-pair importance; bundled with SpecPC (prompt compression) and SpecKV-PC (cascaded strategy with adaptive gamma controller). Anchors `[[1.0.0 PRIM-21]]` Migration Strategy Selection + `[[1.0.0 PRIM-31]]` Iterative Retrieval.
- `[[1.0.0 P-148]]` **LookaheadKV: Parameter-Efficient KV Cache Eviction** (Ahn et al., ICLR 2026, OpenReview RVLMGPXt2i, arXiv:2603.10899) — learnable lookahead tokens + specialized LoRA modules on target model; eviction cost reduced up to 14.5× vs SpecKV-class baselines; improves TTFT. Anchors `[[1.0.0 PRIM-21]]` + `[[1.0.0 PRIM-31]]`.
- `[[1.0.0 P-149]]` **SSD/Saguaro: Asynchronous Speculative Decoding** (Kumar et al., ICLR 2026, OpenReview aL1Wnml9Ef, arXiv:2603.03251) — Saguaro algorithm: draft model predicts next-round verification outcomes while verifier is busy; 30% faster than optimized speculative decoding baselines; up to 5× faster than standard autoregressive decoding. Anchors `[[1.0.0 PRIM-21]]` + `[[1.0.0 PRIM-31]]`.
- `[[1.0.0 P-150]]` **TestPrune: Test-Time Test Set Pruning for LLM-Based Code Repair** (Chen et al., IBM Research, FSE 2026, DOI 10.1145/3808148, arXiv:2510.18270) — coverage-analysis + LLM-prediction hybrid for issue-based test minimization; pipeline-compatible drop-in for SWE-bench-style agentic repair loops; reduces context noise and inference cost. Anchors `[[1.0.0 PRIM-22]]` Comprehension (Observation phase via coverage-driven context pruning) + `[[1.0.0 PRIM-31]]` Iterative Retrieval.

### 12.2 Rejected (3)

- **R1 ABC (Bhardwaj, arXiv:2602.22302)** — REJECTED Q1. arXiv-only February 2026 preprint; no peer-reviewed venue. Pre-existing "Wang et al. ICSE 2026" attribution is a hallucination from earlier sprint; not propagated.
- **R2 NSE Workshop** — REJECTED Q1. NSE 2026 Workshop (co-located with ICSE 2026); off-list workshop venue. Re-verification of Wave-20 U1 confirms the venue claim was the off-list workshop, not the main ICSE 2026 track.
- **R3 Speculative Actions (Liu et al., ICLR 2026)** — REJECTED Q3. Poster-track; mechanism overlap with P-137 SuffixDecoding; no new mechanism beyond incremental pipeline improvement.

### 12.3 Watchlist carried (1)

- **W1 SWE-TRACE** — UNVERIFIED (third carry). arXiv:2604.14820 (April 2026); NeurIPS 2026 notifications scheduled 2026-09-24; SWE-TRACE carried forward for Wave-22 re-verification.

**Cross-reference**:
- Wave-21 candidates memo: `.obsidian/MAgHARCM/research/diary/Wave-21-Candidates.md`
- Wave-21 paper notes: `.obsidian/MAgHARCM/research/papers/P-147..P-150-*.md`
- REJECT registry: `.obsidian/MAgHARCM/Research-Database.json::reject_registry.wave-21` (BLK-08 standing rule applied)
- Watchlist: `.obsidian/MAgHARCM/Research-Database.json::watchlist.wave-21`
- Sprint 2026-09-07 (iter-4) audit block: `.obsidian/MAgHARCM/diary/Sprint-2026-09-07-Handoff-4.md`
- Sprint 2026-09-07 (iter-3) audit block: `.obsidian/MAgHARCM/diary/Sprint-2026-09-07-Handoff-3.md`
- Methodology §7 Wave-21 trigger evaluation (NEW): KV-cache × speculative-decoding substrate reinforcement + static-analysis context pruning for SLM agent comprehension

## 13. Wave-22 SLM-Era Anchors (2026-09-07 iter-6)

2 ACCEPT + 3 REJECT + 1 UNVERIFIED = 6 triaged. Wave-22 opens two substrate loops: P-151 SmartC2Rust closes the C-to-Rust iterative-translation-feedback gap (the C-to-Rust analogue of P-124 Syzygy Go-to-Rust), and P-152 Hallu-Eval / Hallu-Det / Hallu-Shield lands the systematic code-summarization hallucination benchmark + detection + mitigation triplet. The strict program-comprehension-mechanism slot stays open (P-152 is evaluation-substrate, not mechanism); W1 SWE-TRACE carries a fourth time.

### 13.1 Accepted anchors

- `[[1.0.0 P-151]]` **SmartC2Rust: Iterative, Feedback-Driven C-to-Rust Translation via Large Language Models** (Shiraishi, Cao & Shinagawa, ICSE 2026, DOI 10.1145/3744916.3773259, arXiv:2409.10506) — context-aware code segmentation divides large-scale C source into context-bounded units; iterative feedback loop feeds three orthogonal signals (Rust compiler errors, semantic-equivalence diffs, `unsafe`-block counts) until unsafe residuals are minimised and semantic equivalence holds. Anchors `[[1.0.0 PRIM-23]]` + `[[1.0.0 PRIM-29]]` + `[[1.0.0 PRIM-31]]`.
- `[[1.0.0 P-152]]` **Hallucinations in LLM-Based Code Summarization: Unveiling, Detection, and Mitigation** (Liu et al., FSE 2026, DOI 10.1145/3808189, PACMSE Vol 3 Issue FSE) — Hallu-Eval 800-pair benchmark + Hallu-Det detection + Hallu-Shield inference-time mitigation. Anchors `[[1.0.0 PRIM-22]]` + `[[1.0.0 PRIM-25]]`.

### 13.2 Rejected (3)

- **R1 Code vs. Serialized AST** (Dong, Zhao & Harvey, LLM4Code 2026 workshop co-located with ICSE 2026, arXiv:2602.06671) — REJECTED Q1 (off-list workshop venue; empirical comparison study, not method-level mechanism).
- **R2 SmartComment** (Zhang et al., FSE 2026) — REJECTED Q3 (off-axis Solidity smart-contract target; mechanism concrete but outside the MAgHARCM C/Go/Java → Rust substrate).
- **R3 Beyond Accuracy** (Mächtle, Serr, Loose & Eisenbarth, DeepTest 2026 workshop co-located with ICSE 2026, arXiv:2601.12951) — REJECTED Q1 (off-list workshop venue; diagnostic framework with shadow models, not a method-level comprehension mechanism).

### 13.3 Watchlist carried (1)

- **W1 SWE-TRACE** — UNVERIFIED (fourth carry, Wave-19 W2 → Wave-20 W1 → Wave-21 W1 → Wave-22 W1). arXiv:2604.14820 remains preprint-only; NeurIPS 2026 notifications 2026-09-24.

**Cross-reference**:
- Wave-22 candidates memo: `.obsidian/MAgHARCM/research/diary/Wave-22-Candidates.md`
- Wave-22 paper notes: `.obsidian/MAgHARCM/research/papers/P-151..P-152-*.md`
- REJECT registry: `.obsidian/MAgHARCM/Research-Database.json::reject_registry.wave-22` (BLK-08 standing rule applied)
- Watchlist: `.obsidian/MAgHARCM/Research-Database.json::watchlist.wave-22`
- Sprint 2026-09-07 (iter-6) audit block: `.obsidian/MAgHARCM/diary/Sprint-2026-09-07-Handoff-5.md`
- Methodology §7 Wave-22 trigger evaluation: iterative C-to-Rust translation-feedback substrate + code-summarization hallucination evaluation triplet

## 14. Wave-23 SLM-Era Anchors (2026-09-08 iter-1)

4 ACCEPT + 1 REJECT + 1 UNVERIFIED = 6 triaged. P-153 CoReX closes the strict program-comprehension-mechanism residual slot open since Wave-17; P-154 TransAgent and P-155 CodeCureAgent land two FSE 2026 / PACMSE mechanisms; P-156 TestWeaver adds an ICSE 2026 LLM-augmented backward-slicing test-generation anchor. W1 SWE-TRACE carries a fifth time.

### 14.1 Accepted anchors

- `[[1.0.0 P-153]]` **CoReX: Context-Aware Refinement-Based Slicing for Debugging Regression Failures** (Badihi & Rubin, ICSE 2026 Research Track, University of British Columbia) — conditions the slice on the regression-failure context by iteratively refining the candidate slice using observed execution behaviour and recent change history. **Closes the strict program-comprehension-mechanism residual slot carried since Wave-17.** Anchors `[[1.0.0 PRIM-22]]` + `[[1.0.0 PRIM-31]]`.
- `[[1.0.0 P-154]]` **TransAgent: Enhancing LLM-Based Code Translation via Fine-Grained Execution Alignment** (Yuan et al., FSE 2026 / PACMSE Vol 3, DOI 10.1145/3797099, Fudan University) — multi-agent translation pipeline (translator + critic + repairer); fine-grained execution alignment localises error-prone blocks by comparing source vs translated execution behaviour; outperforms UniTrans by up to 33.3% in translation accuracy. Anchors `[[1.0.0 PRIM-23]]` + `[[1.0.0 PRIM-29]]` + `[[1.0.0 PRIM-31]]`.
- `[[1.0.0 P-155]]` **CodeCureAgent: Automatic Classification and Repair of Static Analysis Warnings** (Joos, Bouzenia & Pradel, FSE 2026 / PACMSE Vol 3, DOI 10.1145/3808139, arXiv:2509.11787, CISPA) — agentic code-search + multi-file-edit tool loop; classification + repair twin objective; three-step build+test acceptance heuristic; 96.8% plausible-fix / 86.3% correct-fix on 1,000 SonarQube warnings across 106 Java projects. Anchors `[[1.0.0 PRIM-13]]` + `[[1.0.0 PRIM-22]]` + `[[1.0.0 PRIM-29]]`.
- `[[1.0.0 P-156]]` **TestWeaver: Execution-aware, Feedback-driven Regression Testing Generation with Large Language Models** (Le et al., ICSE 2026, DOI 10.1145/3744916.3787805, arXiv:2508.01255, FPT Software AI Center) — backward slicing for context reduction; close-test retrieval; execution in-line annotations; addresses the coverage-plateau problem in LLM-based regression test generation. Anchors `[[1.0.0 PRIM-22]]` + `[[1.0.0 PRIM-29]]` + `[[1.0.0 PRIM-31]]`.

### 14.2 Rejected (1)

- **R1 AutoCodeSherpa** (Yunbo Lyu et al., ISSTA 2026) — REJECTED Q1 (off-list venue; ISSTA is not on the §7 trigger list and is a main conference, not a workshop). Watch for an ICSE 2027 / FSE 2027 / TOSEM companion paper.

### 14.3 Watchlist carried (1)

- **W1 SWE-TRACE** — UNVERIFIED (fifth carry). arXiv:2604.14820 remains preprint-only; NeurIPS 2026 notifications 2026-09-24.

**Cross-reference**:
- Wave-23 candidates memo: `.obsidian/MAgHARCM/research/diary/Wave-23-Candidates.md`
- Wave-23 paper notes: `.obsidian/MAgHARCM/research/papers/P-153..P-156-*.md`
- REJECT registry: `.obsidian/MAgHARCM/Research-Database.json::reject_registry.wave-23` (BLK-08 standing rule applied)
- Watchlist: `.obsidian/MAgHARCM/Research-Database.json::watchlist.wave-23`
- Sprint 2026-09-08 (iter-1) audit block: `.obsidian/MAgHARCM/diary/Sprint-2026-09-08-Handoff-7.md`
- Methodology §7 Wave-23 trigger evaluation: strict comprehension-mechanism slot closure + multi-agent translation + agentic warning repair + test synthesis

## 15. Wave-24 SLM-Era Anchors (2026-09-09 iter-1)

1 ACCEPT + 1 REJECT + 4 UNVERIFIED = 6 triaged (plus 1 non-event venue note). Wave-24 reinforces the KV-cache × speculative-decoding loop and closes the Wave-23 forward-plan cross-agent KV-cache family with P-157 ReST-KV; the source-to-source modernization and partition-aligned comprehension families were dry at all concluded 2026 venues.

### 15.1 Accepted anchors

- `[[1.0.0 P-157]]` **ReST-KV: Robust KV Cache Eviction with Layer-wise Output Reconstruction and Spatial-Temporal Smoothing** (An, Lu, Zhu, Yu, Zhao, Wu, Tang & Wang, ICLR 2026 Poster, OpenReview PhEHuo7oMm, arXiv:2605.08840) — eviction formulated as a layer-wise output-reconstruction optimization that captures attention-redistribution effects raw-attention scoring misses; EMA temporal smoothing + adaptive spatial windows; +2.58% LongBench, +15.2% RULER, 10.61x decoding-latency reduction at 128k context. Anchors `[[1.0.0 PRIM-21]]` Migration Strategy Selection + `[[1.0.0 PRIM-31]]` Iterative Retrieval Refinement.

### 15.2 Rejected (1)

- **R1 MixKV** (Liu, Gui, Zhang & Zhang, ICLR 2026, arXiv:2510.20707) — REJECTED Q3 (off-axis modality; LVLM multi-modal KV compression while MAgHARCM is text-only code translation).

### 15.3 Watchlist (3 new + 1 carried)

- **W24-W1 SWE-TRACE** — UNVERIFIED (sixth carry). arXiv:2604.14820 remains preprint-only; NeurIPS 2026 notifications 2026-09-24; retire-or-confirm decision mandatory at Wave-25 per the Wave-21 carry rule.
- **W24-W2 MemArt** — UNVERIFIED (first carry). OpenReview YolJOZOGhI in-review ICLR 2026 submission; latent KV-block agent memory adjacent to P-134 RelayCaching.
- **W24-W3 MemDecay** — UNVERIFIED (first carry). arXiv:2607.10582; region-aware eviction + pinning, SLM-relevant (Qwen2.5-1.5B/3B) but arXiv-only.
- **W24-W4 ReCache** — UNVERIFIED (first carry). arXiv:2608.19662; resource-wise attention producing composition-invariant tool-schema KV blocks — the Wave-23 forward-plan cross-agent KV-reuse target and the tool-side analogue of P-134 RelayCaching; fires at Q1 venue confirmation per the P-138 RepairKV precedent.

Non-event venue note: `[[1.0.0 P-01]]` ReCodeAgent has an ASE 2026 main-track version (researchr.org track listing + author page + arXiv:2604.07341); same four-agent mechanism as the anchored FSE version, so not triaged and not registered; P-01 venue string gains the confirmation at the next database touch.

**Cross-reference**:
- Wave-24 candidates memo: `.obsidian/MAgHARCM/research/diary/Wave-24-Candidates.md`
- Wave-24 paper note: `.obsidian/MAgHARCM/research/papers/P-157-ReST-KV-ICLR-2026.md`
- REJECT registry: `.obsidian/MAgHARCM/Research-Database.json::reject_registry.wave-24` (BLK-08 standing rule applied)
