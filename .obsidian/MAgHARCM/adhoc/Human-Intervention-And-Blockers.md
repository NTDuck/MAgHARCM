...
| **Strict program-comprehension-mechanism slot (Wave-22)** | **STILL OPEN (fifth carry into Wave-23)** | The function-level → partition-aligned summary pass gap is partially closed by `[[1.0.0 P-151]]` SmartC2Rust's iterative segmentation loop and `[[1.0.0 P-152]]` Hallu-Eval's systematic hallucination-evaluation triplet. The strict-mechanism slot is the function-level → partition-aligned summary pass that turns per-function comprehension into a partition-aligned architectural summary. Re-scout NeurIPS 2026 (post 2026-09-24) / ICML 2027 (Jan 2027 deadline) / ICLR 2027 (Sep 2026 deadline - embargoed). |
