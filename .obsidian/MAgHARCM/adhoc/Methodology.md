...
Wave 21 trigger evaluation (Sprint 2026-09-07 iter-4): 7 candidates triaged, 4 ACCEPT (P-147..P-150), 3 REJECT (Q1 — R1 ABC arXiv-only no venue; R2 NSE Workshop off-list workshop venue; Q3 — R3 Speculative Actions mechanism overlap with P-137), 1 UNVERIFIED (W1 SWE-TRACE carried Wave-19 → Wave-20 → Wave-21; NeurIPS 2026 notifications pending 2026-09-24). Wave-20 U1 NSE ICML 2026 placeholder re-verified as the NSE 2026 Workshop (co-located with ICSE 2026, off-list); U1 removed from watchlist. Sandbox blocker: git state mutations blocked for the duration of this sprint — see Handoff-4 `Sandbox Blocker` note; all Wave-21 artifacts are untracked this sprint and will land in a single batch commit when the sandbox recovers. Full memo `.obsidian/MAgHARCM/research/diary/Wave-21-Candidates.md`.

Wave 21 SLM-era anchors (4 verified new; 3 rejected; 1 watchlist carried):
| Primitive | SLM Mitigation | Anchor Paper | Verification |
| :--- | :--- | :--- | :--- |
| `[[1.0.0 PRIM-21]]` Migration Strategy Selection | Draft-model-driven lookahead for KV eviction importance; adaptive gamma controller | `[[1.0.0 P-147]]` SpecKV (ICLR 2026, OpenReview 0vbYakkECY) | verified |
| `[[1.0.0 PRIM-31]]` Iterative Retrieval | Parameter-efficient LoRA-modules on target model; learnable lookahead tokens | `[[1.0.0 P-148]]` LookaheadKV (ICLR 2026, OpenReview RVLMGPXt2i) | verified |
| `[[1.0.0 PRIM-21]]` Migration Strategy Selection | Asynchronous draft-verify pipeline; Saguaro algorithm | `[[1.0.0 P-149]]` SSD/Saguaro (ICLR 2026, OpenReview aL1Wnml9Ef) | verified |
| `[[1.0.0 PRIM-22]]` Comprehension | Coverage-driven regression-test minimization; SWE-bench-style drop-in | `[[1.0.0 P-150]]` TestPrune (FSE 2026, DOI 10.1145/3808148) | verified |

Wave 21 anchor list:
- `[[1.0.0 P-147]]` SpecKV (Galim et al. 2026, ICLR 2026, OpenReview 0vbYakkECY, arXiv:2506.08373) — draft-model-driven KV cache eviction; small draft model performs lookahead to predict KV-pair importance; bundled with SpecPC (prompt compression) and SpecKV-PC (cascaded strategy with adaptive gamma controller). Anchors `PRIM-21` Migration Strategy Selection, `PRIM-31` Iterative Retrieval.
- `[[1.0.0 P-148]]` LookaheadKV (Ahn et al. 2026, ICLR 2026, OpenReview RVLMGPXt2i, arXiv:2603.10899) — parameter-efficient KV cache eviction using learnable lookahead tokens + specialized LoRA modules on target model; avoids explicit draft-model generation; eviction cost reduced by up to 14.5x vs. SpecKV-class baselines; improves TTFT. Anchors `PRIM-21` Migration Strategy Selection, `PRIM-31` Iterative Retrieval.
...
## 21. Wave-23 Anchor Table (forward pointer)

Wave-23 anchors are listed inline at §7 Wave 23 trigger evaluation. Wave-23 closes the strict program-comprehension-mechanism residual slot (function-level → partition-aligned summary pass) via P-153 CoReX. Wave-23 priority forward to Wave-24: continue surveying TOSEM 2026 / TSE 2026 concluded programs for AST-representation / code-summarization substrates adjacent to P-130/P-131/P-132 architecture-recovery trio; re-scout for ICSE 2027 / FSE 2027 CFPs that include program-comprehension tracks.

## 22. Wave-23 Forward Plan (forward to Wave-24, not Wave-23 watchlist)

The Wave-23 triage ledger (`Wave-23-Candidates.md` line 57: `ACCEPT = 4, REJECT Q1 = 1 (R1 AutoCodeSherpa ISSTA 2026), UNVERIFIED = 1 (W23-W1 SWE-TRACE fifth carry). Total triaged = 6`) contains only 1 watchlist carry (W23-W1 SWE-TRACE, fifth carry, re-verify after NeurIPS 2026 author notifications 2026-09-24; sixth carry = retire threshold per Wave-21 carry rule). The items below are Wave-24 forward-plan candidates, not Wave-23 watchlist entries.
