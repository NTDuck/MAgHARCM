...
// mockLSPProvider is a programmable LSPProvider used to drive the Navigator
...
// sentinel ErrNoLSPProvider and no sub-calls are made.
func TestNavigatorLookupSymbolNoProvider(t *testing.T) {
	...
	if !errors.Is(res.Error, ErrNoLSPProvider) {
		t.Fatalf("expected ErrNoLSPProvider, got %v", res.Error)
	}
...
// TestNavigatorNewNavigatorNil: NewNavigator(nil) is a valid construction;
// LookupSymbol must produce the no-provider sentinel.
func TestNavigatorNewNavigatorNil(t *testing.T) {
	n := NewNavigator(nil)
	if n == nil {
		t.Fatal("NewNavigator(nil) returned nil")
	}
	if n.Provider != nil {
...
func TestRefCount(t *testing.T) {
	if got := refCount(nil); got != 0 {
		t.Fatalf("refCount(nil) = %d, want 0", got)
	}
	if got := refCount(&tools.ReferencesOutput{}); got != 0 {
		t.Fatalf("refCount(empty) = %d, want 0", got)
	}
	populated := &tools.ReferencesOutput{
		References: []tools.ReferenceMatch{{Line: 1}, {Line: 2}, {Line: 3}, {Line: 4}, {Line: 5}},
	}
	if got := refCount(populated); got != 5 {
		t.Fatalf("refCount(populated) = %d, want 5", got)
	}
}
...
func TestProjectDirOrDot(t *testing.T) {
	...
	for _, tc := range cases {
		if got := projectDirOrDot(tc.in); got != tc.want {
			t.Errorf("projectDirOrDot(%q) = %q, want %q", tc.in, got, tc.want)
		}
	}
}
