...
func TestBuildDirectoryTree(t *testing.T) {
	...
	treeStr, files, err := tools.BuildDirectoryTree(tmpDir, 3)
	if err != nil {
...
`
	...
	structOut, err := tools.ParseFileStructure(cFile)
	if err != nil {
...
func TestLSPProviderProviders(t *testing.T) {
	...
	hover, err := abcoder.GetHover(ctx, &tools.HoverInput{
		Symbol: "Item",
	})
	if err != nil {
...
func TestMultiLanguageExecutionTools(t *testing.T) {
	...
	// No build file
	out, err := tools.ValidateProjectBuild(ctx, tmpDir, "Rust", "cargo")
	if err != nil {
	...
	goOut, err := tools.ValidateProjectBuild(ctx, ".", "Go", "go")
	if err != nil {
