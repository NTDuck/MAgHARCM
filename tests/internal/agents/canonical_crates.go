...
`,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := agents.CanonicalizeCargoToml(tc.input)
			if got != tc.expected {
				t.Fatalf("agents.CanonicalizeCargoToml(%q)\n  got:  %q\n  want: %q", tc.input, got, tc.expected)
			}
		})
	}
}
...
func TestCrateCanonicalHints(t *testing.T) {
	hints := agents.CrateCanonicalHints("Rust")
	if hints == "" {
