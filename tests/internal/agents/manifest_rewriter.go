...
func TestManifestRewriterBasicRewrite(t *testing.T) {
	rw := agents.NewManifestRewriter()
	deps := []agents.SourceDep{
...
func TestManifestRewriterEmptyDeps(t *testing.T) {
	rw := agents.NewManifestRewriter()
	toml, unmapped := rw.Rewrite("minimal", "Java", []agents.SourceDep{})
	if len(unmapped) != 0 {
		t.Fatalf("expected 0 unmapped, got %v", unmapped)
	}
	if !strings.Contains(toml, "[package]") {
...
func TestDetectSourceLang(t *testing.T) {
	...
	for _, tc := range cases {
		got := agents.DetectSourceLang(tc.path)
		if got != tc.expected {
			t.Fatalf("agents.DetectSourceLang(%q) = %q, want %q", tc.path, got, tc.expected)
		}
	}
}
...
func TestQuoteFeatures(t *testing.T) {
	if got := agents.QuoteFeatures(nil); got != "" {
		t.Fatalf("nil features: got %q", got)
	}
	if got := agents.QuoteFeatures([]string{}); got != "" {
		t.Fatalf("empty features: got %q", got)
	}
	if got := agents.QuoteFeatures([]string{"derive"}); got != `"derive"` {
		t.Fatalf("single: got %q", got)
	}
	if got := agents.QuoteFeatures([]string{"derive", "serde1"}); got != `"derive", "serde1"` {
		t.Fatalf("multiple: got %q", got)
	}
}

func TestManifestRewriterRoundTripWithCanonicalize(t *testing.T) {
	// Integration: take a known Java source-dep set, run agents.Rewrite, then
	...
	toml, unmapped := agents.GenerateCargoToml("validator-port", "Java", deps)
	if len(unmapped) != 0 {
		t.Fatalf("expected 0 unmapped for known deps, got %v", unmapped)
	}
	canonical := agents.CanonicalizeCargoToml(toml)
	if canonical == "" {
