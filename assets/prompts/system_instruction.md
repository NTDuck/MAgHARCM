...
3. In Rust, `pub struct Item { pub name: String, pub sell_in: i32, pub quality: i32 }` uses `#[derive(Debug, Clone)]` and `use std::fmt::{self, Display};` for `impl Display for Item` (do not derive Display).
