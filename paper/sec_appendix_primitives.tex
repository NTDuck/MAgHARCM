...
Four of the 32 primitives listed above have been code-integrated into
MAgHARCM as of round 21:
\begin{itemize}
\item NEW-PRIM-28 --- \texttt{internal/agents/checkpoint.go} (round 12, AutoGen-derived interrupt/resume).
\item NEW-PRIM-27 --- \texttt{internal/agents/plateau.go} (round 14, CodaMOSA-derived coverage-guided plateau detector).
\item NEW-PRIM-31 --- \texttt{internal/agents/manifest\_rewriter.go} (round 18, P58 + P60-derived manifest rewriter).
\item NEW-PRIM-26 --- \texttt{internal/agents/navigator.go} (round 19, HyperAgent-derived symbol-aware navigator).
\end{itemize}
The remaining primitives are absorbed into \texttt{research/synthesis/MAgHARCM\_GAP\_TODOS.md} for future landing.
